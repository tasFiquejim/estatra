<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            {{-- <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Welcome back, {{ auth()->user()->first_name }}.</p> --}}
        </div>
        <div class="flex flex-col items-end">
            <div class="flex items-baseline gap-1.5 leading-none">
                ...
                <span class="text-2xl font-semibold tracking-tight text-blue-600 dark:text-blue-400">
                    {{ now()->format('y') }}
                </span>
            </div>
            <div class="mt-1 h-1 w-8 rounded-full bg-blue-600/20 dark:bg-blue-400/20"></div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="mt-4">
                ...
                <p class="mt-1.5 text-xs text-gray-400 dark:text-gray-500">{{ $this->occupiedUnits }} of
                    {{ $this->totalUnits }} units occupied</p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <p class="mt-3 text-2xl font-bold text-gray-800 dark:text-white">
                ৳{{ number_format($this->monthlyIncome, 2) }}</p>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <p class="mt-3 text-2xl font-bold text-gray-800 dark:text-white">
                ৳{{ number_format($this->monthlyExpenses, 2) }}</p>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="flex items-center gap-3">
                <div
                    class="flex h-10 w-10 items-center justify-center rounded-lg {{ $this->netProfit >= 0 ? 'bg-blue-50 dark:bg-blue-500/10' : 'bg-red-50 dark:bg-red-500/10' }}">
                    <svg class="h-5 w-5 {{ $this->netProfit >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-red-600 dark:text-red-400' }}"
            ...
            <p
                class="mt-3 text-2xl font-bold {{ $this->netProfit >= 0 ? 'text-gray-800 dark:text-white' : 'text-red-600 dark:text-red-400' }}">
            ...
            <div class="flex items-center justify-between border-b border-gray-100 px-5 py-4 dark:border-gray-800">
                <div class="flex items-center gap-2">
                    ...
                    @if ($this->expiringSoonLeases->count() > 0)
                        <span
                            class="inline-flex items-center rounded-full bg-orange-100 px-2 py-0.5 text-xs font-medium text-orange-700 dark:bg-orange-500/10 dark:text-orange-400">
            ...
            <div class="p-5">
                @forelse($this->expiringSoonLeases as $lease)
                    ...
                    <div
                        ...
                        <div class="text-right">
                            <span
                                class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $isUrgent ? 'bg-red-100 text-red-700 dark:bg-red-500/10 dark:text-red-400' : 'bg-orange-100 text-orange-700 dark:bg-orange-500/10 dark:text-orange-400' }}">
                                {{ $daysLeft }}d left
                            </span>
                ...
                @empty
                    <div class="flex flex-col items-center py-8 text-center">
                        ...
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">No leases expiring in the next 30 days
                        </p>
                    </div>
                @endforelse
            </div>
