<div class="p-2 mx-auto max-w-7xl">
    ...
    <div class="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
        <h2 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Properties</h2>
                    ...
                    <thead class="bg-gray-50 dark:bg-gray-900/50">
                        <tr>
                            <th class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase cursor-pointer hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"
                                wire:click="sortBy('property_name')">
                        ...
                        @forelse ($this->properties as $property)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        @if($property->property_photo)
                                            <img class="object-cover w-10 h-10 rounded-lg" src="{{ asset('storage/' . $property->property_photo) }}" alt="{{ $property->property_name }}">
                                        @else
                                            <div class="flex items-center justify-center w-10 h-10 bg-gray-100 rounded-lg dark:bg-gray-800">
                                        ...
                                        @endif
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $property->property_name }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400">
                                    {{ $property->address }}
                                    @if($property->city)
                                        <br><span class="text-xs">{{ $property->city }}@if($property->state), {{ $property->state }}@endif</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400">
                                    <span class="inline-flex rounded-full px-2 py-1 text-xs font-semibold 
                                        {{ $property->property_type === 'apartment' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300' : 
                                           ($property->property_type === 'house' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400">
                                    ...
                                    <br><span class="text-xs text-gray-400">{{ $property->created_at->format('g:i A') }}</span>
                                </td>
                                <td class="relative py-4 pl-3 pr-4 text-sm font-medium text-right whitespace-nowrap sm:pr-6">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('property.create', $property) }}" 
