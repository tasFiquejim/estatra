<div class="p-4 mx-auto max-w-7xl md:p-6">
    ...
    <!-- Flash Message -->
    @if (session('success'))
        <div class="px-4 py-3 mt-6 text-sm border rounded-lg border-emerald-300 bg-emerald-50 text-emerald-900 dark:border-emerald-600/40 dark:bg-emerald-900/30 dark:text-emerald-100">
            {{ session('success') }}
        </div>
    @endif

    <!-- Form -->
    <form wire:submit.prevent="save" class="mt-6 rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="p-5 space-y-6 border-t border-gray-100 sm:p-6 dark:border-gray-800">
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <!-- Property Name -->
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Property Name</label>
                    <input
                        type="text"
                        wire:model.defer="property_name"
                        class="shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                        placeholder="e.g. Lakeview Apartment 3B"
                    >
                    @error('property_name') <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="State"
                    >
                    @error('state') <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="12345"
                    >
                    @error('zip_code') <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="Country"
                    >
                    @error('country') <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>

                <!-- Property Type -->
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Property Type</label>
                    <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                        <select
                            wire:model.defer="property_type"
                    ...
                    @error('property_type') <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex items-start gap-4">
                        <div class="w-full">
                            ...
                            >
                            @error('property_photo') <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p> @enderror
                        ...
                        <div class="shrink-0">
                            @if ($property_photo)
                                <img src="{{ $property_photo->temporaryUrl() }}" alt="Preview" class="object-cover w-24 h-24 border border-gray-200 rounded-lg dark:border-gray-800">
                            @elseif ($isEdit && $property?->property_photo)
                                <img src="{{ asset('storage/' . $property->property_photo) }}" alt="Current" class="object-cover w-24 h-24 border border-gray-200 rounded-lg dark:border-gray-800">
                            @else
            ...
            <div class="flex items-center justify-end gap-3 mt-8">
                ...
                >
                    <span wire:loading.remove>Save Property</span>
